...
// import { getMessages, sendMessage } from '../actions/chat'; // 以前作成したServer Actions
...
export const ChatPanel = ({ roomId }: Props) => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        ...
        // ※テスト用のダミーデータ（バックエンドと繋ぐまではこれで確認できます）
        setMessages([
          { id: 1, content: 'よろしくお願いします！', user_id: '1', created_at: new Date(), user: { name: '山田', avatar_url: '' } },
        ]);
      } catch (error) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    setIsSending(true);
    try {
      // 実際はここでServer Actionを呼び出す（※userIdはAuthから取得するか引数で渡す）
      // const newMessage = await sendMessage(roomId, 'dummy-user-id', inputText);
      // setMessages((prev) => [...prev, newMessage]);
      
      // テスト用のダミー追加処理
      setMessages((prev) => [
        ...prev, 
        { id: Date.now(), content: inputText, user_id: 'my-id', created_at: new Date(), user: { name: '自分', avatar_url: '' } }
      ]);
      setInputText(''); // 送信成功したら入力欄を空にする
    } catch (error) {
  ...
  return (
    <div className="flex flex-col h-full w-full relative">
      ...
      >
        {isLoading ? (
          <div className="text-center text-sm text-zinc-500 mt-4">読み込み中...</div>
        ) : messages.length === 0 ? (
          <div className="text-center text-sm text-zinc-500 mt-4">メッセージはまだありません</div>
        ) : (
          messages.map((msg) => {
            const isMyMessage = msg.user_id === 'my-id'; // 実際はログイン中のuserIdと比較します
            return (
              <div key={msg.id} className={`flex ${isMyMessage ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex flex-col max-w-[75%] ${isMyMessage ? 'items-end' : 'items-start'}`}>
                  {/* 送信者名（自分の場合は非表示など調整可能） */}
                  {!isMyMessage && (
                    <span className="text-xs text-zinc-500 mb-1 ml-1">{msg.user.name}</span>
                  )}
                  {/* 吹き出し */}
                  <div 
