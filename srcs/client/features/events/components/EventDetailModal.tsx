...
export const EventDetailModal = ({ roomId, mode, onClose, onSuccess }: Props) => {
    ...
    const handleAction = async (actionType: 'delete' | 'cancel' | 'join') => {
        // 削除・キャンセルの際にダイアログで確認する
        if (actionType === 'delete') {
            if (!window.confirm("この予定を完全に削除しますか？\n（この操作は取り消せません）")) return;
        }
    ...
    // 🌟 編集モードの表示（見た目を詳細モードと合わせるのがコツ！）
    if (isEditing) {
        return (
            <div className="fixed inset-0 bg-black/60 z-[100] flex justify-center items-center p-4" onClick={onClose}>
                <div className="bg-white dark:bg-zinc-900 w-full max-w-md max-h-[90vh] overflow-y-auto rounded-3xl p-6 pb-12 shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
                    ...
                    <EventForm
                      ...
                      onSuccess={() => {
                        onSuccess(); // 親のリストを更新
                        setIsEditing(false); // 編集モードを抜ける（詳細画面に戻る）
                        // 注意：eventData自体を最新にするには再度 loadDetail() を呼ぶ必要がありますが、
                        // 面倒ならそのまま onClose() して一覧に戻してもOKです！
                      }}
                    />
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex justify-center items-center p-4" onClick={onClose}>
            <div className="bg-white dark:bg-zinc-900 w-full max-w-md max-h-[90vh] overflow-y-auto rounded-3xl p-6 shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
                
                {/* 閉じるボタン (✕) */}
                <button onClick={onClose} className="absolute top-4 right-4 text-zinc-400 hover:text-zinc-600">✕</button>
