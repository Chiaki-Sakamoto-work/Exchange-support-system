...
export function ProfileEditForm({ initialData }: { initialData: any }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 bg-white p-6 rounded-3xl border border-zinc-200">
      <div className="space-y-2">
        <label className="text-sm font-bold text-zinc-700">名前</label>
        <input 
          {...register('username')} 
          placeholder="田中 太郎"
      ...
      <div className="space-y-2">
        <label className="text-sm font-bold text-zinc-700">紹介</label>
        <textarea 
          {...register('bio')} 
          rows={4}
      ...
      <div className="space-y-4">
        <div className="flex flex-col">
          <label className="text-sm font-bold text-zinc-700">アレルギー</label>
          <span className="text-xs text-zinc-400">該当するものをすべて選んでください</span>
        </div>
